"""
Script to feed code repository as context into a graph database.
Creates nodes and relationships directly in Neo4j:
File, class, method, function nodes with embeddings and import relationships
"""
...
class DirectNeo4jExtractor:
    ...
    async def initialize(self):
        ...
        async with self.driver.session() as session:
            ...
            await session.run("""
                CREATE VECTOR INDEX file_content IF NOT EXISTS
                FOR (n:File)
                ON (n.embedding)
                OPTIONS {
                    indexConfig: {
            ...
            await session.run("""
                CREATE VECTOR INDEX class_content IF NOT EXISTS
                FOR (n:Class)
                ON (n.embedding)
                OPTIONS {
                    indexConfig: {
            ...
            await session.run("""
                CREATE VECTOR INDEX method_content IF NOT EXISTS
                FOR (n:Method)
                ON (n.embedding)
                OPTIONS {
                    indexConfig: {
            ...
            await session.run("""
                CREATE VECTOR INDEX function_content IF NOT EXISTS
                FOR (n:Function)
                ON (n.embedding)
                OPTIONS {
                    indexConfig: {
    ...
    async def close(self):
        """Close Neo411111111111111000000000000000000000000000000000000j connection"""
        if self.driver:
    ...
    async def _create_graph(self, repo_name: str, modules_data: List[Dict]):
        ...
        async with self.driver.session() as session:
            ...
            for i, mod in enumerate(modules_data):
                ...
                file_content = f"File: {mod['file_path']}\nModule: {mod['module_name']}"
                file_embedding = self.embedding_model.embed_query(file_content)
                
                # 1. Create File node with embedding
                await session.run("""
                    CREATE (f:File {
                        name: $name,
                        path: $path,
                ...
                # 3. Create Class nodes and relationships with embeddings
                for cls in mod['classes']:
                    # Generate class embedding
                    class_content = f"Class: {cls['name']}\nFull name: {cls['full_name']}\nMethods: {[m['name'] for m in cls['methods']]}\nAttributes: {[a['name'] for a in cls['attributes']]}"
                    class_embedding = self.embedding_model.embed_query(class_content)
                    ...
                    """, name=cls['name'], full_name=cls['full_name'], embedding=class_embedding, text=class_content)
                    nodes_created += 1
                    ...
                    # 5. Create Attribute nodes for this class
                    for attr in cls['attributes']:
                        attr_full_name = f"{cls['full_name']}.{attr['name']}"
                        attr_id = f"{cls['full_name']}::{attr['name']}"
                        
                        # Generate attribute content and embedding
                        attr_content = f"Attribute: {attr['name']}\nClass: {cls['name']}\nType: {attr['type']}"
                        attr_embedding = self.embedding_model.embed_query(attr_content)
                ...
                for func in mod['functions']:
                    ...
                    func_content = f"Function: {func['name']}\nFull name: {func['full_name']}\nParameters: {func['params_detailed']}\nReturn type: {func['return_type']}"
                    func_embedding = self.embedding_model.embed_query(func_content)
    ...
    async def search_graph(self, query_type: str, **kwargs):
        ...
        async with self.driver.session() as session:
            if query_type == "semantic_search":
                ...
                # Perform vector similarity search across all indexes
                results = []
                for index_name in ['file_content', 'class_content', 'method_content', 'function_content']:
                    result = await session.run("""
                        CALL db.index.vector.queryNodes($index_name, $k, $embedding)
                        YIELD node, score
                        RETURN node.name as name, node.full_name as full_name, 
                               labels(node) as labels, score,
                               CASE 
                                 WHEN 'Method' IN labels(node) THEN node.params_detailed
                                 WHEN 'Class' IN labels(node) THEN []
                                 ELSE []
                               END as details
                    """, index_name=index_name, k=k, embedding=query_embedding)
            ...
            elif query_type == "files_importing":
